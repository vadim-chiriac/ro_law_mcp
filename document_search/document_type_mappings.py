...
def get_canonical_document_type(doc_type: str, issuer_canonical: str) -> str:
    ...
    logger.info(
        f"Getting canonical doc type for type {doc_type} with issuer {issuer_canonical}"
    )
    issuer_canonical = (
        issuer_canonical.lower()
        .replace("ă", "a")
        .replace("â", "a")
        .replace("ț", "t")
        .replace("ş", "s")
    ...
    logger.info(f"Context key is {context_key}")
